...
Очевидно, что это всё содержимое тега `<form></form>`. Ведь форма будет принимать пользовательский ввод и после отправки должна измениться: либо добавить в разметку ошибки, либо написать, что всё хорошо и письмо ушло.
...
Переменная выводится, теперь мы можем вместо цифр закинуть вызов сниппета FormLister.
...
Вставьте его в любое место формы.
...
```
Вернёмся к контроллеру. Как видите, там также присутствует `formid`. И вот он должен быть равен тому `formid`, который мы задали в вёрстке.
...
```
Обратите внимание на `{{ $data['что-то'] ?? ''}}`. Двойной вопросительный знак - синтаксис php. По сути я говорю шаблонизатору - вставь тут значение, если его нет, ничего не делай.
...
> Мелочь, а приятно:
> FormLister по умолчанию защищается от частой отправки формы. Это неудобно при отладке, и вы можете добавить в вызове сниппета параметры
>  `'protectSubmit' => 0` и `'submitLimit' => 0`.
